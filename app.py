...
if 'df_historial' not in st.session_state:
    data_base = {"sorteo": [3231], "fecha": ["24/05/2026"]}
    for i in range(1, 26):
        data_base[str(i)] = np.random.choice([0, 1], size=1)
...
with st.expander("✍️ Registrar Nuevo Sorteo en el Historial"):
    ...
    with col_m1:
        sorteo_id_man = st.number_input("Número de Sorteo", min_value=3231, value=3231, step=1)
    with col_m2:
        fecha_man = st.text_input("Fecha del Sorteo", value="24/05/2026")
...
with col_izq:
    ...
    if st.button("🚀 Generar y Validar Combinación"):
        ...
        mejor_comb = sorted(df_res.head(14)["Numero" if "Numero" in df_res.columns else "Número"].tolist())
...
# Orden ascendente por número de sorteo para mostrar cronológicamente
df_ordenado = st.session_state.df_historial.sort_values(by="sorteo", ascending=True)
st.dataframe(df_ordenado.tail(10), use_container_width=True)
